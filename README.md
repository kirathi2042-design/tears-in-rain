...
| # | 模式 | 說明 |
|---|------|------|
| 01 | SMOKE 煙霧 | 從畫面邊界慢速吹向中心的陣風前緣，推動煙霧形成物理感渦流 |
| 02 | FIRE 火焰 | 低音與重拍改變火焰高度、亮度、色溫與白熱核心 |
| 03 | RAIN 雨滴玻璃 | 前景雨滴由節拍觸發：重拍落大顆，弱拍落小顆，背景霓虹散景慢速漂移 |
| 04 | LAVA 熔岩燈 | Metaball 熔岩燈，bass 讓蠟球膨脹 |
| 05 | MIST 迷霧雨林 | 雨林剪影、薄霧層、炫光與迷幻色散在林間緩慢變化 |
...
- 風場系統：重拍生成**獨立陣風前緣**（從畫面邊界慢速朝中心推進，帶尾流與 curl 渦旋；連續重音會生成一前一後兩道風，不會把前一道風硬轉向）；人聲中頻化為**漫遊微風**（`uBreeze`，方向緩慢漂移、空間上以 noise 調變成漣漪，絕不整面平移）。煙霧、火焰、熔岩、樹冠全部吃同一個風場
- 雨滴系統：重拍生成**前景落雨事件**（`uRain` 事件佇列），雨滴大小與數量依重音強度變化；玻璃上的細小水珠只作靜態折射，不做連續滑動動畫
